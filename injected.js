// Global variable to fetch local stored sku's
var allBundledSkus = [];
chrome.storage.local.get(['bundleSku'], function (result) {
    console.log('Value currently is ' + result.bundleSku);
    if( result.bundleSku === "" || result.bundleSku == null ){
        allBundledSkus = JSON.parse('""');
    }else{
        allBundledSkus = JSON.parse(result.bundleSku);
    }
    ...
    setupListingsForMaster(data, authToken) {
        async function getListingsForMaster(masters, authToken) {
            ...
            masters.forEach(master => {
                ...
                // let allBundledSkus = JSON.parse(datasku);
